...
def retrieve_AQS_data(year, states="all"):
    ...
    for i, state_code in enumerate(state_code_list, 1):
        ...
        if os.path.exists(f"data/annual_data/{year}/state_data_{state_code}.json"):
            print(f"Annual data for state code {state_code} already exists. Skipping download.")
            continue
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            if attempt < max_retries - 1:
                                ...
                                print(f"      Failed to fetch half-month after {max_retries} attempts")
                                raise
                    
                    if samples_data.get('Data'):
        ...
        with open(f"data/annual_data/{year}/state_data_{state_code}.json", "w") as f:
